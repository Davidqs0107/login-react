...
export const EmpresasAdminPages = () => {
  ...
  const handlePageChange = async (newPage) => {
    const fecha_inicio = document.querySelector(
  ...
  const handleLimpiarDatos = async (empresa) => {
    Swal.fire({
      title: "¿Estás seguro?",
      html: `<p>Esta acción eliminará todos los datos de la empresa <strong>${empresa.nombre}</strong>:</p>
      ...
      if (result.isConfirmed) {
        const response = await limpiarDatosEmpresa(empresa.id);
        if (response) {
          Swal.fire({
            icon: "success",
            title: "Datos eliminados",
            text: `Los datos de la empresa ${empresa.nombre} han sido eliminados correctamente.`,
            timer: 2000,
